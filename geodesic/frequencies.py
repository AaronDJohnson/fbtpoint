...
from mpmath import ellipk, ellipe, ellippi  # TODO: find scipy functions to replace these!

# TODO: after replacing above functions remove float call in returns
# note that scipy doesn't have the third incomplete integral function (ellippi)!
# also speed gains will be minimal since this file is really only used once per mode


def fp_radial_roots(En, Q, aa, slr, ecc, M=1):
    """
...
def fp_polar_roots(En, Lz, aa, slr, x):
    """
...
def mino_freqs_sc(slr, ecc, x):
    ...
    return float(ups_r), float(ups_theta), float(ups_phi), float(gamma)
...
def fp_mino_freqs(r1, r2, r3, r4, En, Lz, Q, aa, slr, ecc, x):
    """
    Mino frequency calculation
